 ...
 public:
  explicit WorkspaceMemoryResource(DLTensor* workspace) {
    if (workspace != nullptr) {
      ...
      TVM_FFI_ICHECK(workspace->ndim == 1 && workspace->dtype.code == kDLUInt && workspace->dtype.bits == 8);
  ...
  void* do_allocate(size_t bytes, size_t alignment) override {
    if (workspace != nullptr) {
      ...
      TVM_FFI_ICHECK(result) << "Failed to allocate " << bytes << " bytes with alignment " << alignment
                    << " bytes.";
